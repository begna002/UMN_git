...
let execute_command tokens =
  match tokens.(0) with
  | "help" -> print_endline help_string
  | "quit" -> quit_now := true
  | "show" ->
    Printf.printf "%s" "--BEG LIST--\n";
  ...
  | "load" ->
    Undolist.set_to_list (Util.strlist_from_file tokens.(1));
  | "mergein" ->
    Undolist.merge_with_list (Util.strlist_from_file tokens.(1));
  | "undo" ->
    let test = Undolist.undo () in
    if test = false then
      print_endline "WARNING: undo list empty, no changes made"
  | "redo" ->
    let test = Undolist.redo () in
    if test = false then
      print_endline "WARNING: redo list empty, no changes made"
  | _ -> Printf.printf "Unknown comand %s" tokens.(0)
;;
