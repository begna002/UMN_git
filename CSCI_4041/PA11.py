...
def optimize_hyperspace_routes(star_list,jump_times):
    ...
    for k in range(n):
        for i in range(n):
            for j in range(n):
                thru_k = Dist[star_list[i].num][star_list[k].num] + Dist[star_list[k].num][star_list[j].num]
                if thru_k < Dist[star_list[i].num][star_list[j].num]:
                    Dist[star_list[i].num][star_list[j].num] = thru_k
                    prev[i][j] = prev[k][j]
    neg_weight_list = find_cycle(star_list, Dist, prev)
    if neg_weight_list != None:
        return neg_weight_list
    

    return Dist


def find_cycle(star_list, Dist, prev):
    n = len(prev)
    neg_weight_list = []
    for i in range(n):
            if Dist[star_list[i].num][star_list[i].num] != 0:
                neg_weight_list.append(star_list[i])
                prev_node = prev[i][i]
                while prev_node != star_list[i]:
                    neg_weight_list.append(prev_node)
                    prev_node = prev[i][prev_node.num]
                neg_weight_list.reverse()
                add = neg_weight_list[0]
                neg_weight_list.append(add)
                return neg_weight_list
...
def Setup_Prev(star_list, jump_times):
    ...
    for i in range(n):
        for j in range(n):
            if i != j and jump_times[star_list[i].num][star_list[j].num] < inf:
                prev[i][j] = star_list[i]
    return prev
