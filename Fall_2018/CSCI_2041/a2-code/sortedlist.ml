...
let rec insert list elem =
  match list with
  | [] -> [elem]
  | head :: tail when head = elem -> head::tail
...
let rec merge lista listb =
  ...
  | [], [] -> []
  | [], (bhead::btail) -> bhead::btail
  | (ahead::atail), [] -> ahead::atail
  | (ahead::atail), (bhead::btail) when ahead = bhead -> ahead::(merge atail btail)
  | (ahead::atail), (bhead::btail) when ahead < bhead -> ahead::(merge atail listb)
  | (ahead::atail), (bhead::btail) when bhead < ahead -> bhead::(merge lista btail)
;;
